...
export const LoginView: React.FC = () => {
  ...
  const [loginMode, setLoginMode] = useState<"STAFF" | "TENANT">("STAFF");
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-amber-400/90 font-medium">
              {language === "ar" ? "نظام الشيكات المرتجعة والقضايا" : "Returned Cheques & Legal Cases"}
            </p>
          </div>
        </div>
        ...
        <div className="w-full max-w-md bg-white rounded-3xl p-8 sm:p-10 shadow-2xl text-slate-800 border border-slate-100 animate-in fade-in zoom-in-95 duration-200">
          <div className="text-center mb-8">
            ...
            <div className="w-14 h-14 bg-amber-50 rounded-2xl border border-amber-200/80 flex items-center justify-center mx-auto mb-4 text-amber-800 shadow-xs">
              {loginMode === "STAFF" ? <ShieldCheck className="w-7 h-7" /> : <User className="w-7 h-7" />}
            </div>
            <h2 className="text-xl font-black text-slate-900 tracking-tight">
              {loginMode === "STAFF" 
                ? (language === "ar" ? "تسجيل دخول الموظفين" : "Staff Sign In")
                : (language === "ar" ? "تسجيل دخول المستأجرين" : "Tenant Sign In")}
            </h2>
            <p className="text-xs text-slate-500 mt-1.5 max-w-xs mx-auto leading-relaxed">
              {language === "ar" 
                ? "يرجى إدخال بياناتك للوصول إلى النظام" 
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="admin or user@sdi.ae"
