...
export const BatchChequeOcrModal: React.FC<BatchChequeOcrModalProps> = ({
  ...
  // Validation Evaluator for Staged Cheques
  const validateCheque = (
    chq: StagedBatchCheque,
    allInBatch: StagedBatchCheque[]
  ): { status: StagedBatchCheque["validationStatus"]; notes?: string } => {
    ...
    const sysDup = checkDuplicateCheque(
      chq.chequeNumber.trim(),
      chq.drawerName || defaultDrawerName || "",
      leaseId,
      tenantId
    );
    if (sysDup) {
      return {
        status: "DUPLICATE",
        notes: isAr
          ? `الشيك مسجل مسبقاً بالنظام (#${chq.chequeNumber})`
          : `Cheque #${chq.chequeNumber} already exists in system`,
      };
    }

    // 3. Duplicate within same batch
    const duplicatesInBatch = allInBatch.filter(
      (item) => item.id !== chq.id && item.chequeNumber && item.chequeNumber.trim() === chq.chequeNumber.trim()
    );
  ...
  const processAndStageExtractedCheques = (extractedItems: any[], sourceImageCount: number) => {
    ...
    const validatedStaged = staged.map((chq) => {
      ...
      return { ...chq, validationStatus: v.status, validationNotes: v.notes };
    });
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        imagePayloads.push(base64);
      }
      const individualResults: any[] = [];
      for (let i = 0; i < imagePayloads.length; i++) {
        setOcrProgressText(
          isAr
            ? `معالجة الشيك رقم (${i + 1}/${imagePayloads.length})...`
            : `Processing cheque (${i + 1}/${imagePayloads.length})...`
        );
        try {
          const single = await extractChequeOCR(imagePayloads[i]);
          if (single) {
            individualResults.push({
              ...(single.data || single.extracted || single),
              imagePreview: imagePayloads[i],
            });
          }
        } catch (singleErr) {
          console.warn("Single extraction item error:", singleErr);
        }
      }

      if (individualResults.length > 0) {
        processAndStageExtractedCheques(individualResults, files.length);
      } else {
        alert(
          isAr
  ...
  const handleSingleRescan = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const ocrResult = await extractChequeOCR(base64);
        ...
        prev.map((item) => {
          if (item.id === singleScanTargetId) {
            const updated: StagedBatchCheque = {
              ...
              amount: normalized.amount > 0 ? normalized.amount : 0,
            ...
            return { ...updated, validationStatus: v.status, validationNotes: v.notes };
          }
          return item;
        })
      );
  ...
  const handleConfirmAndSave = async () => {
    ...
    if (invalidCheques.length > 0) {
      alert(
        isAr
          ? "لا يمكن الاعتماد. يرجى مراجعة وتصحيح كافة الشيكات التي تحتوي على أخطاء أو تعارضات أو بيانات مفقودة أولاً."
          : "Cannot commit. Please review and correct all cheques with errors, mismatches, or missing data first."
      );
      return;
    }
  ...
  return (
    <Modal
      ...
      title={
        isAr
          ? "المسح الذكي لمجموعة الشيكات دفعة واحدة (Multi-Check Batch OCR)"
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="space-y-1">
              <div className="flex items-center gap-2 font-black text-sm text-purple-950">
                ...
                <span>
                  {isAr ? "رفع وقراءة شيكات الدفعات دفعة واحدة:" : "Upload & Scan Cheques Batch:"}
                </span>
                {detectedCount > 0 && (
                  <span className="px-2.5 py-0.5 rounded-full bg-purple-200 text-purple-900 font-mono font-bold text-xs">
                    {isAr ? `تم اكتشاف: ${detectedCount} / ${targetCount}` : `Detected: ${detectedCount} / ${targetCount}`}
                  </span>
                )}
              </div>
              <p className="text-purple-900/80 text-[11px] leading-relaxed">
                {isAr
                  ? "يمكنك رفع صورة واحدة تحتوي على عدة شيكات (ورقة مسح مجمعة) أو رفع عدة صور لشيكات منفصلة. يقوم النظام بالتعرف عليها، وتنسيق أرقامها وبنوكها، وترتيبها تصاعدياً حسب تاريخ الاستحقاق وتوزيعها على جدول الدفعات الفارغة دون المساس بالأقساط المسجلة مسبقاً."
                  : "Upload a single multi-cheque sheet or multiple cheque images. The AI detects all cheques, normalizes UAE banks, sorts chronologically, and sequentially assigns them to open installments."}
              </p>
            </div>
              ...
              >
                ...
                <span>
                  {stagedCheques.length > 0
                    ? (isAr ? "رفع / إعادة مسح مجموعة إضافية" : "Upload Additional Batch")
                    : (isAr ? "اختر صورة الشيكات (Upload Batch)" : "Upload Cheques Batch")}
                </span>
              </button>
            </div>
          </div>
        ...
        {stagedCheques.length > 0 ? (
          ...
          <div className="text-center py-10 bg-slate-50 border border-dashed border-slate-300 rounded-2xl space-y-3">
            ...
            <p className="text-slate-500 text-xs max-w-md mx-auto">
              {isAr
                ? "اضغط على زر (رفع وقراءة شيكات الدفعات) أعلاه لاختيار صورة مجمعة أو عدة صور لشيكات الإيجار."
                : "Click the upload button above to select multi-cheque sheet or individual cheque images."}
            </p>
          </div>
        )}
